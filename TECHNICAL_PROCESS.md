...
How the lookout works. Physics, not poetry. Updated 2026-09-15 to match the scripts.

## Goal of v0

One thin loop for Quebec City:

**ingest → normalize → enrich (proposed) → cluster Issues → rank → display**
...
| Step | Script | Output |
|------|--------|--------|
| 1 Ingest | `scripts/ingest_rss.py` | `data/raw/<source_id>/` append-only |
...
| 5 Rank+HTML | `scripts/rank_display.py` | `latest_ranked.json` + `public/index.html` + ambient twin via `ambient_pulse` |
| 5b Ambient | `scripts/ambient_pulse.py` | `data/pulse/latest_morning.{json,txt}` + `public/morning.html` (same Approaches; no second rank) |
